 ...
 */
function PixiTextInput(text, style, enterHandler) {
    // added 3rd param enterHandler to handle enter keypress
    ...
    //this.backgroundGraphics.defaultCursor = "text";  // DEPRECEATED
    ...
    this.enterHandler = enterHandler;  // assign the custom callback
...
PixiTextInput.prototype.onKeyEvent = function(e) {
	...
	if (e.type == "keydown") {
		switch (e.keyCode) {
            ...
            case 13:  // added this listener for enter keypress
                if (this.textField.text == ' ') return;  // not allowing no name!
                this.enterHandler(this._text);  // passing input text
...
PixiTextInput.prototype.updateText = function() {
	...
	//this.textField.setText(this._text.substring(this.scrollIndex));  // DEPRECEATED
}
